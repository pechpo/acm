...
//为方便操作直接传数组，以O为原点
inline double L_polygen(const V *a, const int &n){
    double res=0;
...
inline void print_polygen(V *a, int &n, bool with_n){
    if (with_n) printf("%d\n", n);
...
inline int lshort(const V &a, const V &b){
    double dot=(a-O)^(b-O);
...
inline void polar_sort(V *a, const int &n, V origin){
    O=origin;
...
void Minkowski_sum(V *a, int n, V *b, int m, V *c, int &N){
...
struct Point{
    ...
    double angle;
    Point(){}
    Point(V p1, int v1):p(p1), v(v1){}
}b[maxn];
struct Query{
    V d;
    int id, ans;
}c[maxn];
bool cmp_(const Query &a, const Query &b){
    return cmp_angle(a.d, b.d);
}
bool cmp__(const Query &a, const Query &b){
    return a.id<b.id;
}
bool cmp___(const Point &a, const Point &b){
    return a.angle<b.angle;
}
int n, m, q;

int main(){
    int T; scanf("%d", &T); V origin;
    while (T--){
        ...
        sort(c, c+q, cmp_);
        origin=get_centroid(a, n);
        for (int i=0; i<n; ++i) a[i]=a[i]-origin;        
        for (int i=0; i<m; ++i) b[i].p=b[i].p-origin;
        for (int i=0; i<q; ++i){ //防止维护的区间里没有点
            b[m++]=Point(c[i].d*1e18, 0);
            b[m++]=Point(c_wise(c[i].d)*1e18, 0);
            b[m++]=Point(-c_wise(c[i].d)*1e18, 0);
        }
        int j=0;
        for (int i=0; i<m; ++i){
            while (!edge_point(a, n, j, L(a[j], b[i].p))) j=nxt(j, n);
            b[i].angle=angle(L(a[j], b[i].p));
        }
        sort(b, b+m, cmp___);
        int l1=0, r1=0, l2=0, r2=0, ans=0;
        L L1, L2, L3;
        for (int i=0; i<q; ++i){
            while (!edge_point(a, n, l1, get_L(c[i].d, a[l1])))
                l1=nxt(l1, n);
            while (!edge_point(a, n, r1, get_L(-c[i].d, a[r1])))
                r1=nxt(r1, n);
            L1=get_L(c[i].d, a[l1]);
            L2=get_L(-c[i].d, a[r1]);
            L3=L(a[r1], a[l1]);
            while (!(!out_hplane(b[pre(r2, m)].p, L2)&&out_hplane(b[r2].p, L2)
                &&in_hplane(b[pre(r2, m)].p, L3)))
                ans+=b[r2].v, r2=nxt(r2, m);
            while (!(!out_hplane(b[l2].p, L1)&&out_hplane(b[pre(l2, m)].p, L1)
                &&in_hplane(b[l2].p, L3)))
                ans-=b[l2].v, l2=nxt(l2, m);
            c[i].ans=ans;
        }
        sort(c, c+q, cmp__);
