#include <cmath>
#include <cstdio>
#include <algorithm>
using namespace std;

const int maxn=1e5+5;
int n, id;
struct Vector{ double x, y; };
struct Point{
    double x, y;
    Vector v;
}p[maxn], O;
Vector operator-(const Point &p1, const Point &p2){
    Vector tmp=(Vector){p1.x-p2.x, p1.y-p2.y};
    return tmp;
}
double operator*(const Vector &v1, const Vector &v2){
    return v1.x*v2.y-v2.x*v1.y;
}
double sqr(double x){ return x*x; }
double dis(const Point &p1, const Point &p2){
    return sqrt(sqr(p1.x-p2.x)+sqr(p1.y-p2.y));
}
bool cmp(const Point &p1, const Point &p2){
    double dot=p1.v*p2.v;
    if (dot>0) return true;
    if (dot==0) return dis(p1, O)<dis(p2, O);  //全共线情况
    return false;
}

int s[maxn], cnt; //栈。从零开始存，存点的编号
double height(int top, int p1, int p2){  //传的是栈的编号，要的是点的编号
    top=s[top]; p1=s[p1%cnt]; p2=s[p2%cnt];
    return abs((p[p2]-p[top])*(p[p1]-p[top]))/(2*dis(p[p1], p[p2]));
}

int main(){
    scanf("%d", &n); id=1;
    for (int i=1; i<=n; ++i){
        scanf("%lf%lf", &p[i].x, &p[i].y);
        if (p[i].y<p[id].y||p[i].y==p[id].y&&p[i].x<p[id].x)
            id=i;
    }
    for (int i=1; i<=n; ++i) p[i].v=p[i]-p[id];
    O=p[id];
    sort(p+1, p+n+1, cmp); int id1, id2;
    for (int i=1; i<=n; ++i){
        while (cnt>=2){
            id1=s[cnt-1]; id2=s[cnt-2];
            if ((p[id1]-p[id2])*(p[i]-p[id1])>0) break;
            --cnt;
        }
        s[cnt++]=i;
    }
    int j=1, tmp;
    double ans=0;
    for (int i=0; i<cnt; ++i){
        while ((tmp=height(i, j, j+1))<height(i, j+1, j+2))
            j=(j+1)%cnt;
        ans=max(ans, dis(p[s[i]], p[s[j]]));
        ans=max(ans, dis(p[s[i]], p[s[(j+1)%cnt]]));
    }
    printf("%d\n", (int)(ans*ans+0.5));
    return 0;
}