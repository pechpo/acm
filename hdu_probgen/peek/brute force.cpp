...
namespace geo_2d{
const double eps=1, INF=1e18, PI=3.14159265358979323846;
...
inline bool straddle(const L &l1, const L &l2){  //l1跨立在l2上
    ...
    if (f1<eps&&f2>-eps||f1>-eps&&f2<eps) return true;
    else return false;
}
...
inline bool on_poly(const V &p, const V *a, const int &n){
    for (int i=0; i<n; ++i)
        if (on_seg(p, L(a[i], a[nxt(i, n)]))) return true;
    return false;
}
inline bool in_poly(const V &p, const V *a, const int &n){  //向左射出射线
...
bool is_intersect(V *a, int &n, const L &l){
    L seg;
    for (int i=0; i<n; ++i){
        seg=L(a[nxt(i, n)], a[i]);
        if (is_intersect2(l, seg)) return true;
    }
    return false;
}
...
vector<V> get_intersection2(V *a, int &n, const L &l){  //线段与多边形交点
    L seg; vector<V> res;
    for (int i=0; i<n; ++i){
        seg=L(a[nxt(i, n)], a[i]);
        if (is_intersect(l, seg)){
            if (parallel(l, seg)){
                if (on_seg(seg.a, l))
                    res.push_back(seg.a);
                if (on_seg(seg.b, l))
...
int main(){
    ...
    //freopen("graphdata.txt", "w", stdout);
    scanf("%d", &T);
    while (T--){
        scanf("%d%d", &n, &m);
        read_polygon(a, n, false);
        cir=0;
        for (int i=0; i<n; ++i) cir+=dis(a[i], a[nxt(i, n)]);
        //printf("%Lf\n", cir);
        step=cir/1e5;  //隔step放一个采样点
        V t, p, P, dir;
        int cnt, tot=0;
        for (int i=0; i<m; ++i){
            scanf("%d", &b[i]);
        ...
        for (int j=0; j<n; ++j){  //枚举线段
            cnt=dis(a[j], a[nxt(j, n)])/step;
            for (int k=0; k<cnt; ++k){  //枚举当前线段上的采样点
                t=a[j]+(a[nxt(j, n)]-a[j])/cnt*k;  //当前采样点
                bool flag=false;
                for (int i=0; i<m; ++i){
                    p=a[b[i]];  //当前窗口点
                    if (!in_poly((p+t)/2, a, n)&&!on_poly((p+t)/2, a, n)) continue;
                    dir=(a[pre(b[i], n)]-p)/1000;
                    if (get_intersection2(a, n, L(p+dir, t)).size()<=2) flag=true;
                    dir=(a[nxt(b[i], n)]-p)/1000;
                    if (get_intersection2(a, n, L(p+dir, t)).size()<=2) flag=true;
                }
                if (flag){
                    //if (T==4) t.print();
                    tot++;
                    //if (tot%100==0) t.print();
                }
            }
        }
        printf("%.5Lf\n", cir*tot/1e5);
