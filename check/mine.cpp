...
inline void polar_sort(V *a, const int &n, const V &origin){
    O=origin;
...
inline void graham(V *a, int &n, bool include_on_edge){
    ...
    for (int i=0; i<=n; ++i){
        while (cnt>=2){
            ...
            if (include_on_edge){
                if (a[id1]!=a[i]&&!turn_right(a[id2], a[id1], a[i%n])) break;
    ...
    if (a[0]==a[cnt-1]) cnt=1;  //特判只有一个点的情况，不出现重复点
    n=cnt;
...
int main(){
    ...
    while (T--){
        m=0;
        scanf("%d%d", &n, &q);
        read_polygen(a, n, false);
        polar_sort(a, n, find_lowest(a, n));
        graham(a, n, false); V tmp; double L=0; int cnt, cnt2=0;
        for (int i=0; i<n; ++i){
            tmp=a[i]-a[pre(i, n)];
            L+=max(abs(tmp.x), abs(tmp.y));
            cnt2+=equal(abs(tmp.x), abs(tmp.y));
        }
        for (int i=0; i<n; ++i)
            for (int j=0; j<5; ++j)
                b[m++]=V(a[i].x+dx[j], a[i].y+dy[j]);
        polar_sort(b, m, find_lowest(b, m));
        graham(b, m, false); cnt=m-n+cnt2;
        double S=S_polygen(a, n);
        int t; double ans;
        for (int i=0; i<q; ++i){
            scanf("%d", &t);
            ans=S+t*L+0.5*t*t*cnt;
            printf("%.1lf\n", ans+0.01);
        }
    }
    return 0;
